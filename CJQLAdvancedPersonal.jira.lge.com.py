...
class CSearchVlm:
    ...
    def connect(self):
        ...
        JIRA_JQL_Search_All(jql_request, fields=["*all"], auth=self.auth_name, passwd=self.auth_passwd, group=str(0), dirname=self.dirname,
                            fileprefix='{s}-first'.format(s=self.fileprefix))  # 해당 JIRA 쿼리문에 해당하는 모든 ticket들의 정보를 가져옴. fields 미입력시 summary 정보만 가져옴
...
def work_func(x, jql, fields, auth, passwd, gap, group, dirname, fileprefix):
    ...
    with open("{dirname}/{fileprefix}-g{g}-{x}.json".format(dirname=dirname, fileprefix=fileprefix, g=group, x=x), "w") as json_file:
        jsondata['fileprefix'] = fileprefix
